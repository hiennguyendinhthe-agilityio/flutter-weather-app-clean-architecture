...
class EBookDetailController extends GetxController {
  ...
  Future<void> fetchBookDetails(String? bookId, Books? initialData) async {
    ...
    if (initialData != null) {
      debugPrint("EBookDetailController: Using initial book data provided.");
    ...
    if (bookId != null) {
      debugPrint(
          "EBookDetailController: Fetching book details for ID: $bookId");
      try {
        final fetchedBook = await _ebookService.getBookById(bookId);
        if (fetchedBook != null) {
          debugPrint("EBookDetailController: Book data fetched successfully.");
          currentBook.value = fetchedBook;
        } else {
          throw 'Book with ID $bookId not found from service.';
        }
      } catch (e) {
        debugPrint("EBookDetailController: Error fetching book details: $e");
