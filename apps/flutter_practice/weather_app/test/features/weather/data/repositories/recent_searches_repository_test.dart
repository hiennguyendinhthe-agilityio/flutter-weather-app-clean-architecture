...
void main() {
  ...
  final tLocation1 = const LocationEntity(
    name: 'London',
    lat: 51.5074,
    lon: -0.1278,
    country: 'GB',
  );
  final tLocation2 = const LocationEntity(
    name: 'Paris',
    lat: 48.8566,
    lon: 2.3522,
    country: 'FR',
  ...
  group('addRecentSearch', () {
    test('should add location to empty list', () async {
      when(() => mockPrefs.getString(tKey)).thenReturn(null);
      when(
        () => mockPrefs.setString(tKey, any()),
      ).thenAnswer((_) async => true);

      await repository.addRecentSearch(tLocation1);

      verify(
        () => mockPrefs.setString(tKey, jsonEncode([tLocation1.toJson()])),
      ).called(1);
    });
    ...
    test('should move existing location to top if already exists', () async {
      ...
      verify(
        () => mockPrefs.setString(tKey, jsonEncode(expectedList)),
      ).called(1);
    });
    ...
    test('should limit list to 5 items', () async {
      ...
      final newLocation = const LocationEntity(
        name: 'New City',
        lat: 99.0,
        lon: 99.0,
        country: 'YY',
      ...
      final captured =
          verify(() => mockPrefs.setString(tKey, captureAny())).captured.first
              as String;
  ...
  group('removeRecentSearch', () {
    test('should remove location from list', () async {
      ...
      verify(
        () => mockPrefs.setString(tKey, jsonEncode([tLocation2.toJson()])),
      ).called(1);
    });
  });
}
