...
class _SalesKpiDonutChartState extends State<SalesKpiDonutChart>
  ...
  void didUpdateWidget(covariant SalesKpiDonutChart oldWidget) {
    ...
    if (oldWidget.percentage != widget.percentage) {
      _oldPercentage = _currentPercentage; // Use current animated value as new starting point
      _currentPercentage = widget.percentage;
    ...
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        final displayPercentage =
            _oldPercentage + (_currentPercentage - _oldPercentage) * _animation.value;
...
class _SalesDonutPainter extends CustomPainter {
  ...
  static const double _cornerRadius = 12.0; // Half of strokeWidth for full roundness
  ...
  void _drawFloatingIcon(Canvas canvas, Offset center, double radius, double angle) {
  ...
  void _drawCentralText(Canvas canvas, Offset center) {
    ...
    pctPainter.paint(
      canvas,
      Offset(center.dx - pctPainter.width / 2, startY),
    );

    subPainter.paint(
      canvas,
      Offset(center.dx - subPainter.width / 2, startY + pctPainter.height + 4.0),
    );
  }
